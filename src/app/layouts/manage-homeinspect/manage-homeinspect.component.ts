...
@Component({
  selector: 'app-manage-homeinspect',
  templateUrl: './manage-homeinspect.component.html',
  styleUrls: ['./manage-homeinspect.component.scss']
})
export class ManageHomeinspectComponent implements OnInit {
  ...
  onRegistration() {
    ...
    this.adminService.AddHomeInspect(formData).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 201) {
          $("#AddModal").modal("hide");
...
async  openEditModal(data){
  ...
  onEditProfile() {
    ...
    this.adminService.EditHomeInspect(formData).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 200) {
  ...
  GetDataList(obj) {
    console.log('obj===================',obj)
    this.adminService.HomeInspectListData(obj).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 200) {
    ...
    swal
      ...
      .then((result) => {
        if (result.value) {
          ...
          this.adminService.DeleteHomeInspect(obj).subscribe(
            (data) => {
              console.log(data);
              if (data.code === 200) {
    ...
    if (this.selection.selected.length > 0) {
      ...
      swal
        ...
        .then((result) => {
          if (result.value) {
            const obj = {
              home_id_arr: this.selection.selected,
            };
            this.adminService.DeleteMultipleHomeInspect(obj).subscribe(
              (data) => {
                console.log(data);
                if (data.code === 200) {
