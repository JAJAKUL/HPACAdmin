...
@Component({
  selector: 'app-manage-contractors-subscription',
  templateUrl: './manage-contractors-subscription.component.html',
  styleUrls: ['./manage-contractors-subscription.component.scss']
})
export class ManageContractorsSubscriptionComponent implements OnInit {
  ...
  addSubscription() {
    ...
    this.adminService.AddContractorsSubscription(obj).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 201) {
          $("#AddModal").modal("hide");
  ...
  editSubscription() {
    ...
    this.adminService.EditContractorsSubscription(obj).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 200) {
  ...
  GetDataList(obj) {
    console.log('obj===================',obj)
    this.adminService.GetContractorsSubscriptionListData(obj).subscribe(
      (data) => {
        console.log(data);
        if (data.code === 200) {
  ...
  changeStatus(data) {
    ...
    let obj = {
      _id: data._id,
      isActive: status,
      schemaName: 'contractorsSubscription',
    };
      console.log('reqbody==================', obj)
